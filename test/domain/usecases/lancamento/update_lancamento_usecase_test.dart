...
void main() {
  ...
  group('UpdateLancamentoUseCase tests', () {
    test('Should return failure when ID is missing', () async {
      ...
      expect(result.exceptionOrNull().toString(), contains('ID do lançamento é obrigatório'));
    });
    ...
    test('Should return failure when item value is <= 0', () async {
      ...
      final extrato = await resolveUseCase.execute(ResolveExtratoFaturaDto(
        origem: origemConta,
        data: createDto.data,
        valor: 100,
        tipo: createDto.tipo,
      ...
      expect(result.isError(), isTrue);
      expect(result.exceptionOrNull().toString(), contains('valor maior que zero'));
    });

    test('Should return failure if original extrato period is closed', () async {
      // 1. Create launch
      final createDto = LancamentoDto(
        descricao: 'Original',
        origem: origemConta,
      ...
      await extratoRepository.update(ExtratoFaturaDto(
      ...
      final result = await useCase.execute(updateDto);
      expect(result.isError(), isTrue);
      expect(result.exceptionOrNull().toString(), contains('Não é possível editar lançamentos de um período encerrado'));
    });

    test('Should succeed when DTO is valid, and recalculate balances for old and new origins', () async {
      ...
      final oldExtratoInDb = allExtratos.firstWhere((e) => e.origem == origemConta);
      expect(oldExtratoInDb.saldoFinal, equals(0.0));

      // New extrato (Conta 2) should have saldoFinal = -150.0 (since it was Despesa of 150)
      final newExtratoInDb = allExtratos.firstWhere((e) => e.origem == origemConta2);
