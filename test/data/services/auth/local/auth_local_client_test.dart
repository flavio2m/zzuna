...
void main() {
  ...
  group('AuthLocalClient', () {
    ...
    test('registerUser returns LoggedUser with a generated local id', () async {
      final result = await authClient.registerUser(
        RegisterUserDto(name: 'New User', email: 'new@example.com', password: 'Aa123456!'),
      );

      expect(result.isSuccess(), isTrue);
      expect(result.getOrThrow().id, startsWith('local_'));
      expect(result.getOrThrow().name, 'New User');
      expect(result.getOrThrow().email, 'new@example.com');
    });

    test('updateUser returns LoggedUser with data from dto', () async {
      final result = await authClient.updateUser(
        UpdateUserDto(id: 'user-1', name: 'Updated User', email: 'updated@example.com'),
      );
