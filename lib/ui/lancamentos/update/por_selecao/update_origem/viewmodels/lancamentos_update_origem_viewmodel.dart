...
class LancamentosUpdateOrigemViewModel extends ChangeNotifier {
  final UpdateLancamentosOrigemUseCase _useCase;
  final ContaRepository _contaRepository;
  final CartaoRepository _cartaoRepository;

  LancamentosUpdateOrigemViewModel(
    this._useCase,
  ...
  late final updateOrigemSelectedCommand =
      Command1<
        Unit,
        ({List<String> lancamentoIds, LancamentoOrigem novaOrigem})
      >(_updateOrigemSelected);

  AsyncResult<Unit> _updateOrigemSelected(
    ({List<String> lancamentoIds, LancamentoOrigem novaOrigem}) params,
  ) async {
    return _useCase.execute(
      lancamentoIds: params.lancamentoIds,
